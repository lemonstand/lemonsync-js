 ...
 */
var accessKeyId, secretAccessKey, sessionToken, bucket, prefix, store;
...
function readConfig() {
  if (fs.existsSync(localConfig)) {
    ...
    try {
      ...
      console.log(
        "Error occured trying to parse lemonsync.json: \r\n" + err.message
      );
      return;
    }
    ...
    if (json.theme_code) {
      ...
      console.log(
        'Field "theme_code" not found in lemonsync.json, please add this field.'
      );
      return;
    }
    ...
    if (json.store) {
      ...
      console.log(
        'Field "store" not found in lemonsync.json, please add this field.'
      );
      return;
    }
    /**
    ...
    if (json.api_token) {
      ...
      console.log(
        'Field "api_token" not found in lemonsync.json, please add this field.'
      );
      return;
    }
    ...
    if (localFilePaths.length == count) {
      for (var key in s3Files) {
        if (s3Files.hasOwnProperty(key)) {
          ...
          if (
            !(localKey in changedRemoteFiles) &&
            !(localKey in matchingFiles)
          ) {
      ...
      console.log(
        'Type "remote" to overwrite your store\'s theme: ' + theme + "\r\n"
      );
  ...
  for (var key in changedFiles) {
    if (changedFiles.hasOwnProperty(key)) {
      try {
        ...
        console.log(
          "Error overwriting local file (" + key + "): " + err.message
        );
      }
    }
  }
...
function uploadLocalToStore(changedFiles) {
  ...
  for (var key in changedFiles) {
    ...
    var themeFileUpdater = s3.upload(params, onFileUpdate).promise();
  ...
  function callback(error, response, body) {
    if (!error && response.statusCode == 200) {
      if (response.statusCode == 401) {
        console.log(
          "The API Access Token isn't valid for " +
            apiHost +
            ". Please check that your Access Token is correct and not expired."
        );
      }
...
function getS3Objects(s3ObjectList, prefix) {
  ...
  if (s3ObjectList.isTruncated) {
    console.log(
      "Too many store files! We truncated this list to our maximum sync size."
    );
  }
  ...
  s3.listObjectsV2(listObjectsV2Params, function(err, objects) {
    if (err) {
      ...
      if (objects.KeyCount == 0) {
        console.log(
          "We couldn't find the theme \"" +
            theme +
            '" in your store. To continue, please create an empty theme with the same name.'
        );
      }
